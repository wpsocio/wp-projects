...
### WordPress CSS Conflict Strategy

WordPress admin CSS conflicts with Tailwind. Because WP admin CSS is **unlayered**, it beats anything inside a CSS cascade layer no matter the specificity — so the plugin stylesheets deliberately emit **no** cascade layers and win on source order instead (our stylesheet loads after core's):

- `globals.css` imports `tailwindcss/theme.css` and `tailwindcss/utilities.css` **without** `layer(...)`. Utilities are imported with the `important` flag, which is what keeps them ahead of WP admin CSS.
- Preflight is scoped by the [`tailwindcss-scoped-preflight`](https://github.com/Roman86/tailwindcss-scoped-preflight) plugin (`isolation-strategy: inside`), declared via `@plugin` inside the `scope-tw-preflight` mixin. The package must be a direct devDependency of each *plugin* — Tailwind resolves `@plugin` relative to the entry `.scss` file.
- Tailwind emits plugin base styles into `@layer base`, so a small PostCSS pass in `@wpsocio/ui/postcss.config` (`wpsocio-unwrap-base-layer`) hoists them out of the layer and to the top of the file. The `* { @apply border-border … }` rule in `globals.css` must stay unlayered so it still overrides the hoisted preflight.
- Each plugin's `styles.scss` uses SCSS mixins from `@wpsocio/ui/mixins.scss`:
  - `reset-wp-common-css` - Small overrides for WP admin defaults that survive preflight
  - `scope-tw-preflight($selectors)` - Scopes Tailwind preflight to the plugin's root element; accepts a comma separated list, used to also cover portalled UI (`.wpsocio-tw` on dialog overlay/content, `[data-radix-popper-content-wrapper]`, `[data-sonner-toaster]`)
